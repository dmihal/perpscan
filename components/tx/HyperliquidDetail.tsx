...
import { TrendingUp, TrendingDown, Landmark, ArrowRightLeft, AlertTriangle } from 'lucide-react';
...
export function TradeDetail({ fills, address }: { fills: Fill[]; address: string }) {
  const first = fills[0];
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        <div className="divide-y divide-border">
          ...
          <DetailRow label="Direction">
            <span className={first.side === 'B' ? 'text-emerald-500' : 'text-destructive'}>{first.dir}</span>
          </DetailRow>
          ...
          <DetailRow label="Total Size"><span className="font-mono">{totalSize.toLocaleString()}</span></DetailRow>
          <DetailRow label="Notional Value">{formatCurrency(totalNotional)}</DetailRow>
          <DetailRow label="Fee">
            <span className="font-mono text-muted-foreground">{totalFee.toLocaleString()} {first.feeToken}</span>
          </DetailRow>
          {totalPnl !== 0 && (
              ...
              <tbody className="divide-y divide-border">
                {fills.map((f, idx) => (
                  <tr key={f.tid || idx} className="hover:bg-muted/50 transition-colors">
                    <td className="px-6 py-3 text-muted-foreground text-xs font-mono">{new Date(f.time).toLocaleString()}</td>
    ...
    <div className="rounded-xl border border-border bg-card p-6">
      ...
      <div className="divide-y divide-border">
        ...
        {d.type === 'accountClassTransfer' && (
          <>
            <DetailRow label="Amount"><span className="font-mono">{formatCurrency(Math.abs(parseFloat(d.usdc)))}</span></DetailRow>
        ...
        {d.type === 'vaultDeposit' && (
          <>
            <DetailRow label="Amount"><span className="font-mono">{formatCurrency(parseFloat(d.usdc))}</span></DetailRow>
            <DetailRow label="Vault"><span className="font-mono text-xs">{d.vault}</span></DetailRow>
          </>
        )}

        {d.type === 'vaultWithdraw' && (
          <>
            <DetailRow label="Net Withdrawn"><span className="font-mono">{formatCurrency(parseFloat(d.netWithdrawnUsd))}</span></DetailRow>
            <DetailRow label="Requested"><span className="font-mono text-muted-foreground">{formatCurrency(parseFloat(d.requestedUsd))}</span></DetailRow>
            <DetailRow label="Commission"><span className="font-mono text-muted-foreground">{formatCurrency(parseFloat(d.commission))}</span></DetailRow>
            <DetailRow label="Vault"><span className="font-mono text-xs">{d.vault}</span></DetailRow>
          </>
        )}
