...
const supportedAccountExchanges = ['hyperliquid', '5507'];
...
export default async function ExchangeAccountPage({ params }: { params: Promise<{ id: string, address: string }> }) {
      ...
      <div className="container mx-auto px-4 py-8 max-w-screen-2xl">
        ...
        <div className="rounded-xl border border-border bg-card p-12 text-center">
          ...
          <p className="text-muted-foreground">
            Account data for {exchange.name} is not yet supported. Currently only Hyperliquid accounts have live data.
          </p>
        </div>
      </div>
    );
  ...
  let hlPnl = 0;
  const positions: { id: string; market: string; side: string; size: number; entryPrice: number; markPrice: number; pnl: number; leverage: number }[] = (hlAccount.assetPositions || []).map((p: any, idx: number) => {
  ...
  try {
    ...
    if (Array.isArray(fills)) {
      history = fills.slice(0, 50).map((fill: any, idx: number) => ({
                  ...
                  <tbody className="divide-y divide-border">
                    {history.map((tx) => (
                      <tr key={tx.id} className="hover:bg-muted/50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-right font-mono">{tx.size}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
